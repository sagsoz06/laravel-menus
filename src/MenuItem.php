...
class MenuItem implements ArrayableContract
{
     ...
     * @return $this
     */
    public function dropdown($title, $url, \Closure $callback, $order = 0, array $attributes = array())
    {
        $properties = compact('title', 'url', 'order', 'attributes');
     ...
     * @return string
     */
    public function getIcon($default = null, $icon = true)
    {
        return !is_null($this->icon) && $icon ? '<i class="' . $this->icon . '"></i>' : $default ? $default : $this->icon;
    }
